...
if(isset($_GET['code'])){ 
    ...
    if(!empty($event_id)){
        ...
        $usersRef = $database->getReference('calendar');

        // Get the data with the key "user1"
        $eventData = $usersRef->getChild($event_id)->getValue();
        ...
        if(!empty($eventData)){ 
            ...
            //     $_SESSION['status_response'] = array('status' => $status, 'status_msg' => "a"); 
            //     $access_token_sess = $_SESSION['google_access_token']; 
            // } catch (Exception $e) {
            //     // handle the error here
            //     $_SESSION['status_response'] = array('status' => $status, 'status_msg' => "b"); 
            //     $access_token_sess = ""; 
            // }
            $access_token_sess = "";
            ...
            // dicoba untuk sementara
            // $data = $GoogleCalendarApi->GetAccessToken("143778673708-8itpfmq078q11u3oph2eok4f22nmd0si.apps.googleusercontent.com", "http://localhost/skripsi/apps/google_calendar2/google_calendar_event_sync.php", "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", $_GET['code']); 
            // $access_token = $data['access_token']; 
            // $_SESSION['google_access_token'] = $access_token; 
