...
if(!isset($_SESSION['access_token'])) {
	header('Location: http://localhost/skripsi/apps/google_calendar_fix/google-login.php');
	exit();	
}
...
<div id="form-container">
	...
	<input type="text" id="event-location" placeholder="Event Location" autocomplete="off" />
	<input type="text" id="event-description" placeholder="Event Description" autocomplete="off" />
...
$("#create-event").on('click', function(e) {
	...
	if(!blank_reg_exp.test($("#event-description").val())) {
		$("#event-description").addClass('input-error');
		error = 1;
	}
