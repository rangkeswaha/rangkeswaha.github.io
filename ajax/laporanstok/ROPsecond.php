...
include('../../db.php');
    ...
    $datahpembelian = $snapshot->getValue();
    ...
    foreach ($datahpembelian as $key => $item) {
        ...
        if ($item['id_Barang'] == $keybarang) {
            ...
            if (date("Ym", strtotime($newDateString)) == date("Ym", strtotime($fetchdatapembelian['tanggal_pembelian']))) {
                ...
                array_push($arrayleadtime, $fetchdatapembelian['lama_pembelian']);
            }
    ...
    $averageleadtime = $leadtime / $matchidcount;
    // echo json_encode($averageleadtime);

    // standard deviation on leadtime
    ...
    $leadtime_standard_deviation = standard_deviation($arrayleadtime);



    
    // get average total penjualan
    ...
    foreach ($filteredData as $key => $item) {
        foreach ($item['list_id_Barang'] as $key => $item) {
            if ($item['key'] == $keybarang) {
                ...
                array_push($arraypenjualan, $item['stok_dijual']);
            }
        }
    }


    $averagepenjualan = $totalm1 / $penjualancount;

    // standard deviation on penjualan
    $penjualan_standard_deviation = standard_deviation($arraypenjualan);


    // echo json_encode($averagepenjualan);

    $sdpenjualancal = $penjualan_standard_deviation * $penjualan_standard_deviation;
    $average_penjualancal = $averagepenjualan * $averagepenjualan;
    $sdleadtimecal = $leadtime_standard_deviation * $leadtime_standard_deviation;
    $totalaboveis = ($averageleadtime * $sdpenjualancal) + ($average_penjualancal * $sdleadtimecal);

    $sqrtofss = sqrt($totalaboveis);
    $SS = 1.28 * $sqrtofss;
    $ROP = $averagepenjualan * $averageleadtime + $SS;

    echo json_encode($ROP);
